...
export class ProductEditComponent {
                ...
                product.options.forEach((option: Option) => {
                    const optionGroup = this.fb.group({
                        ...
                        values: this.fb.array(
                            option.values.map(value => this.fb.group({
    ...
    onSubmit(): void {
        this.productService.update(1, this.editProductForm.value).subscribe(data => {
            this.router.navigate(['/products/detail/' + data.productId], {replaceUrl: true})
        })
    }


    options() {
        return this.editProductForm.get('options') as FormArray;
    }
